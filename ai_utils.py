...
def summarize_package_results(user_name: str, user_age: int, package_name: str, results: list) -> str:
    """Generate a comprehensive summary for a completed package with multiple test results."""
    try:
        llm = get_summurize_result_llm()
        ...
        for result in results:
            test_name = result.get("test_name", "تست")
            summary = result.get("summary", "")
            test_summaries.append(f"## {test_name}\n{summary}\n")
        
        combined_results = "\n\n".join(test_summaries)
        
        system_prompt = """شما یک روانشناس حرفه‌ای هستید که گزارش جامع پکیج تست‌های روانشناسی را تهیه می‌کنید.
وظیفه شما:
1. تحلیل کلی شخصیت بر اساس تمام تست‌ها
2. یافتن الگوها و ارتباطات بین نتایج مختلف
3. ارائه توصیه‌های جامع و کاربردی
4. نوشتن گزارش به زبان فارسی، دوستانه و حرفه‌ای
...
لطفاً یک گزارش جامع و کاربردی تهیه کنید."""
        ...
        LOG.error(f"summarize_package_results failed: {e}")
        return f"⚠️ متأسفانه در تهیه گزارش پکیج خطایی رخ داد.\n\nنتایج تست‌های شما:\n{combined_results}"
...
def create_multimodal_human_message(text: str, media_path: Optional[str] = None, media_type: Optional[str] = None):
    """Create a multimodal message with text and optional media."""
    ...
    try:
        ...
        LOG.error(f"create_multimodal_human_message failed: {e}")
        return HumanMessage(content=text)


def process_user_info_multimodal(name_age_message, personal_info_message) -> str:
    """Process user info from multimodal messages."""
    try:
        llm = get_llm()
        
        system_prompt = """شما یک دستیار هوشمند هستید که اطلاعات کاربر را از پیامهای او استخراج میکنید.
اطلاعات مورد نیاز:
- نام
- سن
- شغل (اگر ذکر شده)
...
def extract_user_profile(user_id: int, question1_text: str, question2_text: str, 
                         question1_media=None, question2_media=None) -> Optional[Dict[str, Any]]:
    """Extract structured user profile from responses."""
    try:
        llm = get_llm()
        
        system_prompt = """شما یک دستیار هوشمند هستید که پروفایل کاربر را از پاسخهای او استخراج میکنید.
اطلاعات را به صورت JSON با این ساختار برگردانید:
{
  "name": "نام کاربر",
  "age": عدد سن,
  "occupation": "شغل",
  "interests": ["علاقه1", "علاقه2"],
  "bio": "خلاصه اطلاعات شخصی"
}"""

        user_prompt = f"""سوال 1: نام و سن خود را بگویید
پاسخ: {question1_text}

سوال 2: درباره خودتان بگویید
پاسخ: {question2_text}
...
لطفاً پروفایل را استخراج کنید."""
        ...
        LOG.error(f"extract_user_profile failed: {e}")
        return None
